...
(function($) {
    // 1. Define the HTML with an SVG icon instead of a text arrow
    var sideButtonHtml = `
        <div id="cert-side-widget" class="closed">
            <div id="cert-tab">
                <div class="icon-wrapper">
                    <!-- Modern Certificate/Badge SVG Icon -->
                    <svg class="cert-svg" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path class="badge-path" d="M9 12L11 14L15 10M12 2L15.09 3.5L18.41 2.5L19.5 5.59L22.5 7.09L21.5 10.41L23 13.5L20.5 16.5L20.5 19.5L17.5 20.5L15.5 23L12 22L8.5 23L6.5 20.5L3.5 19.5L3.5 16.5L1 13.5L2.5 10.41L1.5 7.09L4.5 5.59L5.59 2.5L8.91 3.5L12 2Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <!-- Close (X) SVG - Hidden by default via CSS -->
            ...
            <div id="cert-content">
                <div class="cert-header">Certificat</div>
                <div class="cert-body">
                    <img src="assets/img/salamatouna.png" alt="Salamatouna" class="cert-img">
                    <img src="assets/img/imanor.png" alt="IMANOR NM ISO 9001" class="cert-img">
                </div>
            </div>
        </div>
    `;
    ...
    // 2. Append to body
    $('body').append(sideButtonHtml);

    // 3. Toggle Logic
    $(document).on('click', '#cert-tab', function() {
        $('#cert-side-widget').toggleClass('closed');
    });
          ...
          $(document).on('click', '.goToContact', function (e) {
              ...
              const contactSection = $('#contact-sec');

              if (contactSection.length) {
                  // We are on the HOME page -> Just scroll
                  $('html, body').animate({
                      scrollTop: contactSection.offset().top - 80
                  }, 800);
              } else {
                  // We are on ANOTHER page -> Go home with a scroll parameter
                  // Change 'index.html' to '/' if your site is in the root
                  window.location.href = 'index.html?action=scroll-to-contact';
              }
          });

          // --- 2. THE PAGE LOAD CHECK ---
          // This runs every time a page is opened
          const urlParams = new URLSearchParams(window.location.search);
          if (urlParams.get('action') === 'scroll-to-contact') {

              // Wait for page and translations to be ready
              setTimeout(() => {
                  const target = $('#contact-sec');
                  if (target.length) {
                      $('html, body').animate({
                          scrollTop: target.offset().top - 80
                      }, 800);

                      // Optional: Clean the URL (removes the ?action... from the browser bar)
                      window.history.replaceState({}, document.title, window.location.pathname);
                  }
              }, 600); // 600ms delay to let animations/translations finish loading
          }



          /*******************************************/
      // 1. RUN THIS IMMEDIATELY (Before DOMContentLoaded if possible)
...
function applyLanguage(lang) {
    ...
    elements.forEach(el => {
        ...
        if (data[key]) {
            ...
            // Lettering.js Fix
            if (el.classList.contains('circle-title-anime')) {
                if (window.jQuery && typeof jQuery.fn.lettering === 'function') {
                    $(el).lettering(); 
                }
            }
        }
...
function initLanguage() {
    const langToApply = localStorage.getItem('selectedLang') || 'fr';
    
    // Try to apply immediately
    const success = applyLanguage(langToApply);

    // If it failed (translations not loaded yet), keep trying every 50ms
    if (!success) {
        const retryInterval = setInterval(() => {
            if (window.translations && window.translations[langToApply]) {
...
if (document.readyState === 'loading') {
    ...
    initLanguage();
          ...
          (function ($) {
              ...
              // 1. CLONE the original container and give it a new ID
              var $mobilePopup = $('#headerLinksPopup').clone().attr('id', 'headerLinksPopupMobile');

              // 2. CREATE a Close Button and add it inside the cloned popup
              var $closeBtn = $('<button class="mobile-popup-close-btn"><i class="fa-solid fa-xmark"></i></button>');
              $mobilePopup.prepend($closeBtn); // Adds it to the top of the popup

              // 3. CREATE a dark overlay background
              var $overlay = $('<div id="mobilePopupOverlay"></div>');

              // 4. APPEND both the overlay and the popup to the <body>
              $('body').append($overlay).append($mobilePopup);

              // 5. OPEN the popup when the info button is clicked
              $(document).on('click', '#infoToggleBtn', function (e) {
                  e.preventDefault();
                  $mobilePopup.addClass('show-popup');
                  $overlay.addClass('show-popup');
              });

              // 6. CLOSE logic
              function closePopup() {
                  $mobilePopup.removeClass('show-popup');
              ...
              // Close when clicking the dark overlay (outside the popup)
              $(document).on('click', '#mobilePopupOverlay', function () {
                  closePopup();
              });
